...
class WorkingHour extends Model
{
    ...
    protected $fillable = ['date', 'start_time', 'finish_time', 'recurrence', 'employee_id'];
    ...
    {
        return $this->hasMany(WorkingHour::class, 'event_id', 'id');
    }

    public function event()
    {
        return $this->belongsTo(WorkingHour::class, 'event_id');
    }

    public function saveQuietly()
    {
        // getting the dispatcher instance (needed to enable again the event observer later on)
        $dispatcher = WorkingHour::getEventDispatcher();
        // disabling the events
        WorkingHour::unsetEventDispatcher();
        // perform the operation you want
        $this->save();
        // enabling the event dispatcher
        WorkingHour::setEventDispatcher($dispatcher);
    }
}
